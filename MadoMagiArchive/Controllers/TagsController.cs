...
namespace MadoMagiArchive.Controllers
{
    ...
    public class TagsController(DataDbContext dataDb, UserContext userContext) : ControllerBase
    {
        [HttpGet]
        ...
        {
            var query = dataDb.Tags.Where(x => userContext.ReadLevel >= (x.Permission & 0x00ff0000) / 0x10000 || userContext.Id == x.Owner);

            if (type != null) query = query.Where(x => x.Type == type);

            if (searchKey != null)
            {
                var tagNameQuery = dataDb.TagNames
                    .Where(x => x.Name.ToLower().Contains(searchKey.ToLower()))
                    .Select(x => x.TagId);

                var tagDescriptionQuery = dataDb.Tags
        ...
        {
            var tag = await dataDb.Tags.Include(x => x.Names).SingleOrDefaultAsync(x => x.Id == id);
        ...
        public async Task<ActionResult<ApiResponse>> PostTag(Tag tag)
        {
            ...
            await dataDb.Tags.AddAsync(tag);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<int>>>> PutTags(TagsUpdateDTO update)
        {
            ...
            var entities = await dataDb.Tags
